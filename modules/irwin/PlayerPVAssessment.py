...
class PlayerPVAssessment():
  @staticmethod
  def combineInputs(X):
    playerandgamesfnn = tf.contrib.layers.stack(X, tf.contrib.layers.fully_connected, [25, 8, 4, 2], scope="mainnetwork")
    return tf.reshape(playerandgamesfnn, [-1, 2])

  @staticmethod
  def inference(X):
    return tf.nn.softmax(PlayerPVAssessment.combineInputs(X))

  @staticmethod
  def loss(X, Y):
    comb = PlayerPVAssessment.combineInputs(X)
  ...
  @staticmethod
  def inputs():
    inputList = PlayerPVAssessment.readCSV(800, [[0.0]]*16)
  ...
  def readCSV(batchSize, recordDefaults):
    filename_queue = tf.train.string_input_producer(['data/classified-player-pvs.csv'])
  ...
  def learn(minTrainingSteps, incTrainingSteps):
    ...
    with graph.as_default():
      with tf.Session(graph=graph) as sess:
        X, Y = PlayerPVAssessment.inputs()
        ## initliase graph for running
        totalLoss, evaluation, comp = PlayerPVAssessment.loss(X, Y)
        trainOp = PlayerPVAssessment.train(totalLoss)
        ...
        ckpt = tf.train.get_checkpoint_state('modules/irwin/models/player-pvs')
        ...
        logging.warning("training player pvs to: "+str(trainingSteps))
        ...
        for step in range(initialStep, trainingSteps):
          ...
          if step % 1000 == 0:
            ...
            saver.save(sess, 'modules/irwin/models/player-pvs/model', global_step=step)
        coord.request_stop()
        coord.join(threads)
        saver.save(sess, 'modules/irwin/models/player-pvs/model', global_step=trainingSteps)
  ...
  def applyNet(batch):
    ...
    with graph.as_default():
      with tf.Session(graph=graph) as sess:
        a = tf.placeholder(tf.float32, shape=[None, 15])
        infer = PlayerPVAssessment.inference(a)
        feedDict = {a: batch}
        ## initliase graph for running
