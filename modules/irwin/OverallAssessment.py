...
class OverallAssessment():
  @staticmethod
  def combineInputs(X):
    playerandgamesfnn = tf.contrib.layers.stack(X, tf.contrib.layers.fully_connected, [4, 3, 2], scope="mainnetwork")
    return tf.reshape(playerandgamesfnn, [-1, 2])

  @staticmethod
  def inference(X):
    return tf.nn.softmax(OverallAssessment.combineInputs(X))

  @staticmethod
  def loss(X, Y):
    comb = OverallAssessment.combineInputs(X)
  ...
  @staticmethod
  def inputs():
    inputList = OverallAssessment.readCSV(800, [[0.0]]*3)
  ...
  def readCSV(batchSize, recordDefaults):
    filename_queue = tf.train.string_input_producer(['data/classified-overall-assessment.csv'])
  ...
  def learn(minTrainingSteps, incTrainingSteps):
    ...
    with graph.as_default():
      with tf.Session(graph=graph) as sess:
        X, Y = OverallAssessment.inputs()
        ## initliase graph for running
        totalLoss, evaluation, comp = OverallAssessment.loss(X, Y)
        trainOp = OverallAssessment.train(totalLoss)
        ...
        ckpt = tf.train.get_checkpoint_state('modules/irwin/models/overall-assessment')
        ...
        logging.warning("training overall assessment to: "+str(trainingSteps))
        ...
        for step in range(initialStep, trainingSteps):
          ...
          if step % 1000 == 0:
            ...
            saver.save(sess, 'modules/irwin/models/overall-assessment/model', global_step=step)
        coord.request_stop()
        coord.join(threads)
        saver.save(sess, 'modules/irwin/models/overall-assessment/model', global_step=trainingSteps)
  ...
  def applyNet(batch):
    ...
    with graph.as_default():
      with tf.Session(graph=graph) as sess:
        ...
        infer = OverallAssessment.inference(a)
        feedDict = {a: batch}
        ## initliase graph for running
