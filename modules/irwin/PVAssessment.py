...
class PVAssessment():
  ...
  @staticmethod
  def inference(X):
    return tf.nn.softmax(PVAssessment.combineInputs(X))

  @staticmethod
  def loss(X, Y):
    comb = PVAssessment.combineInputs(X)
  ...
  @staticmethod
  def inputs():
    inputList = PVAssessment.readCSV(800, [[0.0]]*6)
  ...
  def readCSV(batchSize, recordDefaults):
    filename_queue = tf.train.string_input_producer(['data/classified-pvs.csv'])
  ...
  def learn(minTrainingSteps, incTrainingSteps):
    ...
    with graph.as_default():
      with tf.Session(graph=graph) as sess:
        X, Y = PVAssessment.inputs()
        ## initliase graph for running
        totalLoss, evaluation, comp = PVAssessment.loss(X, Y)
        trainOp = PVAssessment.train(totalLoss)
        ...
        ckpt = tf.train.get_checkpoint_state('modules/irwin/models/pvs')
        ...
        for step in range(initialStep, trainingSteps):
          ...
          if step % 1000 == 0:
            ...
            saver.save(sess, 'modules/irwin/models/pvs/model', global_step=step)
        coord.request_stop()
        coord.join(threads)
        saver.save(sess, 'modules/irwin/models/pvs/model', global_step=trainingSteps)
  ...
  def applyNet(batch):
    ...
    with graph.as_default():
      with tf.Session(graph=graph) as sess:
        ...
        infer = PVAssessment.inference(a)
        feedDict = {a: batch}
        ## initliase graph for running
