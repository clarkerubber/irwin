...
class GameCombinedAssessment():
  @staticmethod
  def combineInputs(X):
    playerandgamesfnn = tf.contrib.layers.stack(X, tf.contrib.layers.fully_connected, [4, 2], scope="mainnetwork")
    return tf.reshape(playerandgamesfnn, [-1, 2])

  @staticmethod
  def inference(X):
    return tf.nn.softmax(GameCombinedAssessment.combineInputs(X))

  @staticmethod
  def loss(X, Y):
    comb = GameCombinedAssessment.combineInputs(X)
  ...
  @staticmethod
  def inputs():
    inputList = GameCombinedAssessment.readCSV(800, [[0.0]]*3)
  ...
  def readCSV(batchSize, recordDefaults):
    filename_queue = tf.train.string_input_producer(['data/classified-games-combined.csv'])
  ...
  def learn(minTrainingSteps, incTrainingSteps):
    ...
    with graph.as_default():
      with tf.Session(graph=graph) as sess:
        X, Y = GameCombinedAssessment.inputs()
        ## initliase graph for running
        totalLoss, evaluation, comp = GameCombinedAssessment.loss(X, Y)
        trainOp = GameCombinedAssessment.train(totalLoss)
        ...
        ckpt = tf.train.get_checkpoint_state('modules/irwin/models/game-combined')
        ...
        logging.warning("training game-combined to: "+str(trainingSteps))
        ...
        for step in range(initialStep, trainingSteps):
          ...
          if step % 1000 == 0:
            ...
            saver.save(sess, 'modules/irwin/models/game-combined/model', global_step=step)
        coord.request_stop()
        coord.join(threads)
        saver.save(sess, 'modules/irwin/models/game-combined/model', global_step=trainingSteps)
  ...
  def applyNet(batch):
    ...
    with graph.as_default():
      with tf.Session(graph=graph) as sess:
        ...
        infer = GameCombinedAssessment.inference(a)
        feedDict = {a: batch}
        ## initliase graph for running
