...
class PVOverallAssessment():
  @staticmethod
  def combineInputs(X):
    playerandgamesfnn = tf.contrib.layers.stack(X, tf.contrib.layers.fully_connected, [10, 5, 2], scope="mainnetwork")
    return tf.reshape(playerandgamesfnn, [-1, 2])

  @staticmethod
  def inference(X):
    return tf.nn.softmax(PVOverallAssessment.combineInputs(X))

  @staticmethod
  def loss(X, Y):
    comb = PVOverallAssessment.combineInputs(X)
  ...
  @staticmethod
  def inputs():
    inputList = PVOverallAssessment.readCSV(800, [[0.0]]*4)
  ...
  def readCSV(batchSize, recordDefaults):
    filename_queue = tf.train.string_input_producer(['data/classified-pvs-overall.csv'])
  ...
  def learn(minTrainingSteps, incTrainingSteps):
    ...
    with graph.as_default():
      with tf.Session(graph=graph) as sess:
        X, Y = PVOverallAssessment.inputs()
        ## initliase graph for running
        totalLoss, evaluation, comp = PVOverallAssessment.loss(X, Y)
        trainOp = PVOverallAssessment.train(totalLoss)
        ...
        ckpt = tf.train.get_checkpoint_state('modules/irwin/models/pvs-overall')
        ...
        logging.warning("training pvs-overall to: "+str(trainingSteps))
        ...
        for step in range(initialStep, trainingSteps):
          ...
          if step % 1000 == 0:
            ...
            saver.save(sess, 'modules/irwin/models/pvs-overall/model', global_step=step)
        coord.request_stop()
        coord.join(threads)
        saver.save(sess, 'modules/irwin/models/pvs-overall/model', global_step=trainingSteps)
  ...
  def applyNet(batch):
    ...
    with graph.as_default():
      with tf.Session(graph=graph) as sess:
        ...
        infer = PVOverallAssessment.inference(a)
        feedDict = {a: batch}
        ## initliase graph for running
