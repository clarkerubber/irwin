...
class GamesAssessment():
  @staticmethod
  def combineInputs(X):
    layer2 = tf.contrib.layers.fully_connected(X, 32)
    layer3 = tf.contrib.layers.fully_connected(layer2, 16, activation_fn=tf.nn.sigmoid)
    layer4 = tf.contrib.layers.fully_connected(layer3, 16, activation_fn=tf.nn.sigmoid)
  ...
  @staticmethod
  def inference(X):
    return tf.nn.softmax(GamesAssessment.combineInputs(X))

  @staticmethod
  def loss(X, Y):
    comb = GamesAssessment.combineInputs(X)
  ...
  def learn(minTrainingSteps, incTrainingSteps):
    ...
    with graph.as_default():
      with tf.Session(graph=graph) as sess:
        X, Y = GamesAssessment.inputs()
        ## initliase graph for running
        totalLoss, evaluation, comp = GamesAssessment.loss(X, Y)
        trainOp = GamesAssessment.train(totalLoss)
