...
class PlayerAssessment():
  ...
  @staticmethod
  def inputs():
    inputList = PlayerAssessment.readCSV(800, [[0.0]]*3)
  ...
  def learn(minTrainingSteps, incTrainingSteps):
    ...
    with graph.as_default():
      with tf.Session(graph=graph) as sess:
        ...
        ckpt = tf.train.get_checkpoint_state('modules/irwin/models/player')
        ...
        for step in range(initialStep, trainingSteps):
          ...
          if step % 1000 == 0:
            ...
            saver.save(sess, 'modules/irwin/models/player/model', global_step=step)
        coord.request_stop()
        coord.join(threads)
        saver.save(sess, 'modules/irwin/models/player/model', global_step=trainingSteps)
