...
class GamePVAssessment():
  @staticmethod
  def combineInputs(X):
    playerandgamesfnn = tf.contrib.layers.stack(X, tf.contrib.layers.fully_connected, [15, 10, 2], scope="mainnetwork")
    return tf.reshape(playerandgamesfnn, [-1, 2])

  @staticmethod
  def inference(X):
    return tf.nn.softmax(GamePVAssessment.combineInputs(X))

  @staticmethod
  def loss(X, Y):
    comb = GamePVAssessment.combineInputs(X)
  ...
  @staticmethod
  def inputs():
    inputList = GamePVAssessment.readCSV(800, [[0.0]]*16)
  ...
  def readCSV(batchSize, recordDefaults):
    filename_queue = tf.train.string_input_producer(['data/classified-game-pvs.csv'])
  ...
  def learn(minTrainingSteps, incTrainingSteps):
    ...
    with graph.as_default():
      with tf.Session(graph=graph) as sess:
        X, Y = GamePVAssessment.inputs()
        ## initliase graph for running
        totalLoss, evaluation, comp = GamePVAssessment.loss(X, Y)
        trainOp = GamePVAssessment.train(totalLoss)
        ...
        ckpt = tf.train.get_checkpoint_state('modules/irwin/models/game-pvs')
        ...
        logging.warning("training game PVs to: "+str(trainingSteps))
        ...
        for step in range(initialStep, trainingSteps):
          ...
          if step % 1000 == 0:
            ...
            saver.save(sess, 'modules/irwin/models/game-pvs/model', global_step=step)
        coord.request_stop()
        coord.join(threads)
        saver.save(sess, 'modules/irwin/models/game-pvs/model', global_step=trainingSteps)
  ...
  def applyNet(batch):
    ...
    with graph.as_default():
      with tf.Session(graph=graph) as sess:
        ...
        infer = GamePVAssessment.inference(a)
        feedDict = {a: batch}
        ## initliase graph for running
