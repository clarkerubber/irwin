...
class PVLosingAssessment():
  ...
  @staticmethod
  def inference(X):
    return tf.nn.softmax(PVLosingAssessment.combineInputs(X))

  @staticmethod
  def loss(X, Y):
    comb = PVLosingAssessment.combineInputs(X)
  ...
  @staticmethod
  def inputs():
    inputList = PVLosingAssessment.readCSV(800, [[0.0]]*6)
  ...
  def readCSV(batchSize, recordDefaults):
    filename_queue = tf.train.string_input_producer(['data/classified-pvs-losing.csv'])
  ...
  def learn(minTrainingSteps, incTrainingSteps):
    ...
    with graph.as_default():
      with tf.Session(graph=graph) as sess:
        X, Y = PVLosingAssessment.inputs()
        ## initliase graph for running
        totalLoss, evaluation, comp = PVLosingAssessment.loss(X, Y)
        trainOp = PVLosingAssessment.train(totalLoss)
        ...
        ckpt = tf.train.get_checkpoint_state('modules/irwin/models/pvs-losing')
        ...
        logging.warning("training losing pvs to: "+str(trainingSteps))
        ...
        for step in range(initialStep, trainingSteps):
          ...
          if step % 1000 == 0:
            ...
            saver.save(sess, 'modules/irwin/models/pvs-losing/model', global_step=step)
        coord.request_stop()
        coord.join(threads)
        saver.save(sess, 'modules/irwin/models/pvs-losing/model', global_step=trainingSteps)
  ...
  def applyNet(batch):
    ...
    with graph.as_default():
      with tf.Session(graph=graph) as sess:
        ...
        infer = PVLosingAssessment.inference(a)
        feedDict = {a: batch}
        ## initliase graph for running
